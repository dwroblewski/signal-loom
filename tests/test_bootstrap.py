...
def test_codex_plugin_root_takes_precedence_over_claude_root(tmp_path):
    ...
    for example in (ROOT / "config").glob("*.example.yaml"):
        (codex_root / "config" / example.name).write_text(example.read_text())
    ...
    assert r.returncode == 0, r.stderr
    assert (codex_root / "config" / "signal-loom.yaml").exists()
    assert not (claude_root / "config" / "signal-loom.yaml").exists()
